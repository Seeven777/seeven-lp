...
function Dashboard({ all, onNavigate, onSeed, seeding }) {
  ...
  return <>
    <section className="admin-command-hero"><div><span>CONTROL ROOM / V8</span><h1>O portfólio fica melhor quando você sabe <b>o que falta.</b></h1><p>Este painel mede completude editorial — não performance de marketing. Priorize mídia real e cases claros antes de adicionar mais efeitos.</p></div><div className="admin-score" style={{'--score':`${score*3.6}deg`}}><div><strong>{score}%</strong><span>COMPLETUDE<br/>DO CMS</span></div></div></section>
...
export default function AdminApp() {
  ...
  const exportBackup=()=>{const blob=new Blob([JSON.stringify({version:'8.0',exported_at:new Date().toISOString(),data:all},null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`seeven-cms-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url)}
