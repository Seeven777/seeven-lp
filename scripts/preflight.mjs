...
assert(/function ExperienceGate/.test(app), 'Gateway de descoberta V10.5 ausente.')
assert(/function Hero/.test(app) && /v105-brand-map/.test(app), 'Hero / mapa de marcas V10.5 ausente.')
...
assert(/function WorkArchive/.test(app) && /id=\"brands\"/.test(app), 'Arquivo de marcas V10.5 ausente.')
assert(/function Partners/.test(app) && /v105-partner-rail/.test(app), 'Rede cinética de parceiros V10.5 ausente.')
...
// Deploy / repo
assert(pkg.version === '10.5.0', `Versão esperada 10.5.0; atual ${pkg.version}.`)
...
console.log('SEE7VEN V10.5 / PREFLIGHT OK')
